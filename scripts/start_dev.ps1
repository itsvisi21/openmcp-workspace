Write-Host "Starting OpenMCP Development Environment..."
...
$backendCommand = "cd '$(Join-Path $rootDir "apps\backend")'; .\.venv\Scripts\Activate.ps1; `$env:PYTHONPATH='src'; uvicorn src.app.main:app --reload --port 8000"
Start-ServiceWindow -Title "Backend Server" -Command $backendCommand

# Start MCP server
$mcpCommand = "cd '$(Join-Path $rootDir "apps\mcp-server")'; .\.venv\Scripts\Activate.ps1; uvicorn app.main:app --reload --port 8001"
Start-ServiceWindow -Title "MCP Server" -Command $mcpCommand

# Start dashboard
$dashboardCommand = "cd '$(Join-Path $rootDir "apps\dashboard")'; npm run dev"
Start-ServiceWindow -Title "Dashboard" -Command $dashboardCommand

Write-Host "`nDevelopment environment started!"
